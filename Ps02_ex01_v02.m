...
tic
p_explicitEuler = fn_KolmogForwEqn(xx,tt,options, theta, sigma, xbar, p0,  1,2);
toc
display('Running implicitEuler with central Derivative...')
display('Running time = ...')
tic
%p_implicitEuler = fn_KolmogForwEqn(xx,tt,options, theta, sigma, xbar, p0,  2,1);
toc
tic
%p_matlabSolver = fn_KolmogForwEqn(xx,tt,options, theta, sigma, xbar, p0, 2,1);
...
ylabel('P(x,t=5)')
hold on;
%plot(x_grid,p_implicitEuler(:,5),'-*b','Linewidth',0.01)
%plot(x_grid,p_matlabSolver(:,5),'--g','Linewidth',2)
